...
Future<Message> getWhatHappenedInArcResponse(Message input) async {
  var lowercaseMessage = input.message.toLowerCase();
  var whathappened = lowercaseMessage
      .split('what happened in the ')
      .join()
      .split(' arc')
      .join()
      .split('?')
      .join();
  var url = '$baseurl/Events/$whathappened';
  final uri = Uri.parse(url);
  final response = await http.get(uri);
  if (response.statusCode == 404) {
    // Return a message indicating that more information was not found
    return Message(
        sender: 'Nico Robin', message: 'Sorry, the information was not found.');
  }
