...
namespace HR.Controllers
{
    ...
    public class BountiesController : ControllerBase
    {
        ...
        public BountiesController(IGraphClient client)
        {
            _client = client;
        }
       ...
       public async Task<IActionResult> Get(){
        ...
        [EnableCors]
        [HttpGet("{character}")]
       public async Task<IActionResult> GetByName(string character)
        {
             var response = await _client.Cypher.Match("(d:Character)")
                                                   .Where((Character d) => d.name == character)
                                                   .OrWhere((Character d) => d.alias == character)
